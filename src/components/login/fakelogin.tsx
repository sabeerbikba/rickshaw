...
export default function FakeLoginPage() {
    ...
    const [dummyUserName, setDummyUserName] = useState<string>("");                         const [dummyPassword, setDummyPassword ] = useState<string>("");

    const handleClick = async () => {
        setLoading(true);
        // Request only sended when those input is empty: and click Login button
        if (!dummyUserName && !dummyPassword) {                                                 await fetch("/login/api", {
            method: "POST"                                                                      });
        setLoading(false);
        }
    }
                                                                                        
        // styles not given yet that's why use p elements                                   return (
        <div>
        <p> sabeer bikba</p>
        <p> sabeer bikba</p>
        <p> sabeer bikba</p>
