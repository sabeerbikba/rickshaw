...
app.post('/api/upload', upload, async (req, res) => {
    console.log(`New POST req hit with ${req.files.length} images at ${Date()}`);
    if (req.files.length <= 0) {
        console.log('no images uplaoded!');
        console.log(req.files);
        return res.status(400).send('No file uploaded');
    ...
    try {
        ...
        for (const file of req.files) {
            ...
            console.log('everything okay inserting to db');
            const result = await collection.insertOne({
                id: collectionCount + 1,
                imageName: file.originalname,
                srcUrl: response.data.url,
...
app.get('/api/images', async (req, res) => {
    ...
    if (!imageNames || !Array.isArray(imageNames)) return res.status(400).json({
        error: 'No image names provided or invalid format.'
    });
...
app.get('/api/loadimages', async (req, res) => {
    const { page } = req.query;
    console.log(`Req hit pageNumber: ${page} at ${Date()}`);
    const perPage = 5;
    const skip = (page - 1) * perPage;
